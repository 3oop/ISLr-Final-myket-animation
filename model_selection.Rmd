...
pred_linear_regression <- predict(linear_regression_model, test_data)
sse_linear_regression <- sum((pred_linear_regression - test_data$Amtiaz)^2)
sst <- sum((test_data$Amtiaz - mean(test_data$Amtiaz))^2)
...
sse_ridge <- sum((pred_ridge - test_data$Amtiaz)^2)
sst <- sum((test_data$Amtiaz - mean(test_data$Amtiaz))^2)
...
sse_lasso <- sum((pred_lasso - test_data$Amtiaz)^2)
sst <- sum((test_data$Amtiaz - mean(test_data$Amtiaz))^2)
r2_lasso <- 1 - sse_lasso/sst
```
#XG Boost
```{r}
xgb_model = xgboost(data = x_train, label = y_train, nrounds = 30, objective = "reg:squarederror", eta = 0.1, max_depth = 8)
pred_xgb <- predict(xgb_model, x_test)
sse_xgb <- sum((pred_xgb - test_data$Amtiaz)^2)
sst <- sum((test_data$Amtiaz - mean(test_data$Amtiaz))^2)
